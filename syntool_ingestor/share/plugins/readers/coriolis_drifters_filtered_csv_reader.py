...
class DriftersCSVReaderPlugin(IReaderPlugin):
    ...
    def can_handle(self, source_format):
        """ """
        return 'coriolis_drifters_filtered_csv' == source_format.lower()
    ...
    def _extract_metadata(self, drifter_id, drifter_data, cfg):
        """ """
        dataset_prefix = cfg['input_options'].get('dataset', 'Filtered_Coriolis')
        metadata = { 'begin_datetime': drifter_data[0]['date']
                   , 'end_datetime': drifter_data[-1]['date']
                   , 'product': cfg['input_options'].get('product', 'Filtered_Coriolis')
    ...
    def _parse_csv(self, path, field_names, no_data_value):
        ...
        with open(path, "rb") as csv_file:
            ...
            for row in csv_reader:
                ...
                date=datetime.strptime(row['DATE'] ,date_fmt_input)
                date_str = datetime.strftime(date, date_fmt) #row['date'] #,date_fmt_input
                ...
                for field in to_extract:
                    u_value = row['speed'+field]
                    if no_data_value in [u_value]:
                        # Skip row
                        missing_data = True
                        break;
                    value = u_value
